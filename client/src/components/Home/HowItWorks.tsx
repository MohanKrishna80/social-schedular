        ...
        <section id="how-it-works" className="py-24 bg-white dark:bg-slate-950">
            <div className="max-w-4xl mx-auto px-4 sm:px-6">
                <div className="text-center mb-16">
                    ...
                    <p className="mt-5 text-gray-500 max-w-lg mx-auto leading-relaxed dark:text-slate-400">No complicated onboarding, no steep learning curve. Just connect, create, and grow.</p>
                </div>

                <div className="space-y-6">
                    {steps.map((s, i) => (
                        <div key={s.step} className="flex gap-6 items-start">
                            <div className="shrink-0 size-12 rounded-2xl bg-red-50 border border-red-100 flex items-center justify-center dark:bg-red-500/10 dark:border-red-500/20">
                                <span className="text-sm font-medium text-red-500">{s.step}</span>
                            </div>
                            <div className="pt-1">
                                <h3 className=" text-slate-900 mb-1 dark:text-slate-100">{s.title}</h3>
                                <p className="text-slate-500 text-sm leading-relaxed dark:text-slate-400">{s.description}</p>
                            </div>
                            {i < steps.length - 1 && (
                                <div className="hidden sm:block ml-auto shrink-0 self-center">
                                    <ArrowRightIcon className="size-4 text-slate-200 dark:text-slate-700" />
                                </div>
                            )}
                        </div>
                    ))}
