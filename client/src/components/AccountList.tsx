...
const AccountList = ({ accounts, onDisconnect }: AccountListProps) => {
  ...
  if (accounts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-6 bg-white rounded-2xl border-2 border-dashed border-slate-200">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {accounts.map((account, index) => {
        ...
        return (
          <div
            key={index}
            className="group bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-5 flex items-center gap-4 transition-all"
          >
